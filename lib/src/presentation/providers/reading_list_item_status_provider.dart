...
final readingListItemEffectiveReadStatusProvider = Provider.autoDispose.family<AsyncValue<bool>, ReadingListItem>((ref, item) {
...
final readingListEffectiveStatusProvider = Provider.autoDispose.family<AsyncValue<({int readCount, int totalCount, double progress})>, ReadingList>((ref, list) {
  final itemsStatuses = list.items.map((item) => ref.watch(readingListItemEffectiveReadStatusProvider(item))).toList();
  ...
  if (isLoading) {
    return AsyncValue.data((readCount: readCount, totalCount: totalCount, progress: progress));
  }
  
  return AsyncValue.data((readCount: readCount, totalCount: totalCount, progress: progress));
});

final seriesAllIssuesReadProvider = FutureProvider.autoDispose.family<bool, int>((ref, seriesId) async {
  ...
  while (hasNext) {
    final page = await metronRepo.getSeriesIssueList(seriesId, page: currentPage);
  ...
  final statusMap = await ref.watch(collectionIssueStatusMapProvider.future);

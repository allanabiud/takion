...
final bulkScrobbleProvider = NotifierProvider<BulkScrobbleController, AsyncValue<void>>(
    ...
    state = await AsyncValue.guard(() async {
      try {
        ...
        for (final issueId in issueIds) {
          ...
          final wasCollected = existing?.ownershipStatus == LibraryOwnershipStatus.owned;
          final wasWishlisted = existing?.ownershipStatus == LibraryOwnershipStatus.wishlist;
          
          final targetIsRead = markAsRead ?? (dateRead != null || (existing?.isRead ?? false));
          final targetIsCollected = wasCollected; // Keep existing collection status
          ...
          await libraryRepository.upsertItem(
            ...
            firstReadAt: targetIsRead ? (existing?.firstReadAt ?? readAt) : existing?.firstReadAt,
          ...
          if (targetIsRead && !wasRead) {
            ...
            final readLogs = await libraryRepository.getReadLogsByIssueId(issueId);
            final firstLog = readLogs
                .where((log) => log.readAt.toUtc().toIso8601String() == existing!.firstReadAt!.toUtc().toIso8601String())
                .cast()
                .toList();

...
class ReadingTrendPoint {
  const ReadingTrendPoint({required this.label, required this.count, required this.date});
...
DateTime _atStartOfWeek(DateTime date) =>
...
final profileInsightsProvider = FutureProvider.autoDispose.family<ProfileInsights, ProfileFilter>((
  ref,
  filter,
) async {
  ...
  final readsInPeriod = readOwned
      .where((item) => item.firstReadAt != null && (startDate == null || !item.firstReadAt!.isBefore(startDate)))
      .length;
  ...
  if (filter == ProfileFilter.week && startDate != null) {
    for (var i = 0; i < 7; i++) {
      ...
      final count = readDates
          ...
          .length;
      readingTrends.add(
        ReadingTrendPoint(label: DateFormat.E().format(day), count: count, date: day),
      );
    }
    ...
    for (var i = 1; i <= 12; i++) {
      ...
      final count = readDates
          ...
          .length;
      readingTrends.add(
        ReadingTrendPoint(label: DateFormat.MMM().format(month), count: count, date: month),
      );
    }
  ...
  final ownedIssueIds = owned
      ...
      .toSet()
      .toList();
  final sampleIssueIds = ownedIssueIds.take(50).toList();

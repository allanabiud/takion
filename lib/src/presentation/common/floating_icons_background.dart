...
class _FloatingIconsPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    for (final icon in icons) {
      final double dx =
          icon.centerPosition.dx + icon.floatDirection.dx * (progress - 0.5);
      final double dy =
          icon.centerPosition.dy + icon.floatDirection.dy * (progress - 0.5);

      final offset = Offset(dx * size.width, dy * size.height);

      final opacity = (1.0 - (icon.depth * 0.6)) * baseColor.a;

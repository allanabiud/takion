...
6. **Centralize duplicated constants** — `Duration(milliseconds: 500)` debounce (9 files), 5-min keepAlive timer (9+ files), and raw settings keys (`'metron_api_token'`, `'has_seen_onboarding'`, `'drive_sync_enabled'`, `'superhero_api_token'`, `'superhero_integration_enabled'`, `'superhero_use_image'`, …) into `core/constants/`. **[DONE]** — created `SettingsKeys` and `AppDurations` constants in `core/constants/`.
7. **De-duplicate library screens** — `library_screen.dart:135-312` ("Reading Suggestion" vs "Rate Suggestion" are the same loading/error/data Column). One parameterized section widget. **[DONE]** — extracted `_IssueSuggestionSection` widget in `library_screen.dart`.

  ...
  test('upsert then get round-trip preserves values', () async {
    ...
    await dao.upsert(
      SuperheroCharacterCacheCompanion.insert(
        metronCharacterId: Value(42),
        superheroId: 70,
        superheroName: 'Batman',
        imageUrl: Value('https://example.com/batman.jpg'),
        powerstatsJson: Value('{"combat":100}'),
        updatedAt: now,
      ),
    );
  ...
  test('upsert on existing key overwrites', () async {
    ...
    await dao.upsert(
      SuperheroCharacterCacheCompanion.insert(
        ...
        imageUrl: Value('https://example.com/spidey.jpg'),
        updatedAt: DateTime.now().toUtc(),
      ),
    );
